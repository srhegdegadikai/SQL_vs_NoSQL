...
Only databases/data-stores that are compatible with both, the Linux environment and the Windows environment were selected for the experiment. Thus, popular NoSQL data-stores such as Redis and Cassandra were excluded from the initial experiment. MySQL, Elasticsearch and MongoDB were selected for the experiment. R code, SQL and JSON queries used to insert and query the data are inside the various R-scripts. Comments have been inserted to ascertain the functioning of code snippets. "analysis\_script.R" contains all the R code that has been used in the analysis to visualize and tidy the data.

#### Data -

The data selected for the experiment is an Apache-Weblog that contains traces for seven month's of HTTP requests made to the University of Saskatchewan's WWW server. This data is available [(http://ita.ee.lbl.gov/html/contrib/Sask-HTTP.html)](http://ita.ee.lbl.gov/html/contrib/Sask-HTTP.html) as an open source log-archive. The data-set contains one table with the following attributes,
...
Let us perform a Student's t-test to determine if there is significant difference between the performance given by Elasticsearch and the other two.

    ## 
    ##  Welch Two Sample t-test
    ## 
    ## data:  time by database
    ## t = -45.139, df = 49.004, p-value < 2.2e-16
    ## alternative hypothesis: true difference in means is not equal to 0
    ## 95 percent confidence interval:
    ##  -1.591777 -1.456087
    ## sample estimates:
    ## mean in group Elasticsearch         mean in group MySQL 
    ##                 0.009161773                 1.533093808

The above result gives a lot of output that might be hard gauge at first, but we need to concentrate mainly on two metrics, "alternative hypothesis" and the "p-value". Our null hypothesis is that the mean run-time between the groups(two databases) is equal, here it is indicating that we need to reject that hypothesis. Thus we can assuredly claim that at least for the given type of queries Elasticseacrh performs remarkably better than a MySQL database even when we exclude the type of queries MySQL is bad at. We can safely say so since the "p-value" - probability of getting the results the other way is very small.
...
The above two plots show the effect of indexes on MySQL very clearly. After adding the indexes the run-times have clearly gone down for all the queries, thus bringing down the mean value. Even the outliers, the queries that had group-by-aggregate clauses are being executed way faster than before.
